...
MOFTy is a multimodal Bayesian factor analysis framework that jointly infers shared latent factors across multiple data modalities, while optionally incorporating continuous covariates such as time or spatial coordinates. It supports joint modeling with Gaussian, Bernoulli, and Poisson likelihoods, typically used for real-valued, binary, and count data, respectively, and naturally accommodates missing observations without prior imputation. Suppose we are given <i>M</i> data matrices (views) <i>Y</i><sup>(1)</sup>, ..., <i>Y</i><sup>(<i>M</i>)</sup>, where each matrix <i>Y</i><sup>(<i>m</i>)</sup> contains <i>D</i><sub><i>m</i></sub> features observed across <i>N</i> samples that are shared across views. For a given number of latent factors <i>L</i>, MOFTy represents each view through a low-rank factorization <i>W</i><sup>(<i>m</i>)</sup><i>Z</i>, where <i>Z</i> is the shared <i>L</i> &times; <i>N</i> latent factor matrix and <i>W</i><sup>(<i>m</i>)</sup> is the view-specific <i>D</i><sub><i>m</i></sub> &times; <i>L</i> weight matrix. For Gaussian likelihoods, the model includes an additive independent Gaussian noise term <i>&epsilon;</i><sup>(<i>m</i>)</sup> with feature-specific variance parameters learned during training:

<p align="center">
<i>Y</i><sup>(<i>m</i>)</sup> = <i>W</i><sup>(<i>m</i>)</sup><i>Z</i> + <i>&epsilon;</i><sup>(<i>m</i>)</sup>.
</p>
...
MOFTy reports total and factor-wise variance decompositions for each view, computed separately for the combined, CFM, and non-CFM components. For Gaussian likelihoods, this corresponds to the standard <i>R</i><sup>2</sup>. For Bernoulli likelihoods, we implemented Tjur's <i>R</i><sup>2</sup> (Tjur, 2009).

### Features of MOFTy
- Multimodal factor analysis with Gaussian / Bernoulli / Poisson likelihoods
...
- Support for metric Gaussian variational inference (MGVI) and geometric variational inference (geoVI)
- Parallelization via the Message Passing Interface (MPI, https://www.mpi-forum.org/docs/)
- GPU acceleration via CuPy (Okuta et al., 2017)
- Compatibility with MOFA2 (R package; mofax, Python) and integration with the scverse ecosystem via muon for preprocessing and downstream analysis (Argelaguet et al., 2018; Argelaguet et al., 2020; Velten et al., 2022; Bredikhin et al., 2022; Virshup et al., 2023)
...
If an untrained MOFA or MEFISTO model file is used, the MOFA2 R package uses `group1` as the group identifier, while MEFISTO-based workflows may use `single_group`.
When preprocessing with mofax or muon, the identifier is usually `group1`.
This matters only for input parsing, since MOFTy currently supports only single-group models.
...
- We recommend making sure that feature names are unique within and across data modalities/views.
...
If neither is set to `true`, the identity link is used, which requires non-negativity
constraints on both factors and weights.
...
| `CFM_SCALING_INIT_VALUES` | `str` | `fixed` | Initialization strategy for the CFM scaling parameters: `fixed` sets every factor to `BETA_INIT`, `random` draws from the prior |
...
A padding value of <i>p</i> expands the computational pixel grid to <i>p</i> times its original extent along each axis, so that data-constrained grid points are centered within the padded domain.
...
Alternatively, random seeds and reference seeds for posterior alignment can be provided explicitly, as shown in `config_files/config_*_master.yaml`.
...
For each run, we draw <i>N</i> mirror samples (2<i>N</i> samples in total). MOFTy provides an option to change the sampling scheme after a given number of global iterations.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `N_START_SAMPLES` | `int` | `4` | Number of mirror samples drawn per iteration in the initial phase |
| `N_FINAL_SAMPLES` | `int` | `4` | Number of mirror samples drawn per iteration after the initial phase |
| `N_START_SAMPLES_ITERATIONS` | `int` | `5` | Number of global iterations using `N_START_SAMPLES` |
...
Set the `SYN_DATA` flag to `true` in your config file to generate synthetic data. This corresponds to drawing one sample from the forward model according to the following parameters.


| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `N_FACTORS` | `int` | `5` | Number of factors |
| `DATA_SAMPLES` | `int` | `200` | Number of samples; only used when `USE_COVARIATES` is `false` (otherwise the sample count follows from `COVARIATES_SHAPE`) |
...
| `PADDING` | `int` | `1` | Padding factor used for the harmonic grid |
| `SEED`| `int`| `42` | Seed |
| `TARGET_SNR`| `float` | `1` | Target signal-to-noise ratio for views with Gaussian observation model |
| `CFM_SCALING_VALUES` | `list[float]` | `[0.99, 0.75, 0.5, 0.25, 0.01]` | Scaling of `Z_CFM` vs `Z_non-CFM`, the length of the list has to equal `N_FACTORS`|
| `SYN_SCALING` | `float` | `0.6` | Scaling the standard deviation of all variables except those corresponding to the inverse-gamma prior on weights |
...
To reconstruct synthetic data, set the `SYN_DATA_REC` flag to `true` in your reconstruction configuration file.
...
Note that for `SYN_DATA_REC`, the reference seed is ignored during posterior alignment. Instead, the ground-truth factors are used to derive the orthogonal transformations of the latent factors (see [MOFTy publication](#citing) for more details).
...
Example for `output/output_gbm_cfm_4`:

```text
output/output_gbm_cfm_4
├── log_files/
...
└── runs_combined_factors_4/
    ...
    │       std_{rna,protein}_mean{,_hist}
    │     }
    └── ot_matrix_seed_<seed>.csv (orthogonal transformation matrix to align factors to the reference seed)
```
...
R
```
Execute the following command in `R`. If you are asked for updates, enter `n` for `none`.
...
```
Then execute the following command. If you are asked for updates, enter `n` for `none`.
```R
...
<a id="system-2"></a>
- **System 2:** Linux workstation (Ubuntu 24.04.4 LTS), two 32-core Intel Xeon Gold 6430 processors (two threads per core, 128 logical cores total), 1 TB main memory, two NVIDIA A100 GPUs with 80 GB of memory each.

We remark that different hardware configurations may lead to slightly different results. On macOS, we ran all scripts with `nohup /usr/bin/time -l python3 <script + CLI args> &> script.log &`. On Linux, the command is `nohup /usr/bin/time --verbose python3 <script + CLI args> &> script.log &`. The `--rp` flag controls how many seed models run in parallel. With the default `--rp 1`, the four seed models train sequentially; `--rp 2` trains two at a time, and `--rp 4` trains all four in parallel. The `<file_name>_master.yaml` configuration files used below contain the exact random seeds used in the MOFTy paper. To generate new random seeds instead, run MOFTy with `<file_name>.yaml`.
...
- 1D synthetic data reconstruction (900 pixels): 2.6 GB total peak memory usage, corresponding to approximately 650 MB per model; 1.9 hours total training time. 
- 2D synthetic data reconstruction (30x30 pixels): 2.3 GB total peak memory usage, corresponding to approximately 575 MB per model; 1.8 hours total training time.
...
On [System 2](#system-2), we also trained up to eight models in parallel (four models/random seeds per GPU).
Each model (seed) was initialized on a single CPU core.
For each dataset, total training time across the four parallel seed runs was measured as wall-clock time, and peak memory usage was measured as maximum resident set size and maximum allocated GPU memory.
...
For model comparison, we also trained MEFISTO models (fixed random seed 42) on the glioblastoma dataset with 4 factors and a varying number of inducing points.
...
To generate the synthetic data used in the [MOFTy publication](#citing) with 900 pixels (1D) and 30x30 pixels (2D), activate the `mofty` environment and run:
```bash
...
First run `notebooks_py/preprocessing_gbm.ipynb`, using the `muon_analysis` environment. Then activate the `mofty` environment and run:
...
For the MEFISTO, MOFTy, and PCA comparison analysis, first train the models:
```bash
cd comparison_gbm
...
We also trained models with 4, 6, 8, and 10 factors on [System 2](#system-2) with the `mofty_cupy` environment:
```bash
